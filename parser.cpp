...
ASTNode* Parser::Expression1()
{
	...
	case Equal:
		...
		tnode = Term();
		e1node = Expression1();
		return CreateNode(OperatorEqual, e1node, tnode);

	case LessThan:
		GetNextToken();
		tnode = Term();
		e1node = Expression1();
		return CreateNode(OpLessThan, e1node, tnode);
	}
...
ASTNode* Parser::CreateIfNode(ASTNodeType type, ASTNode* left, ASTNode* right, ASTNode* center)
{
	ASTNode* node = new ASTNode;
	node->Type = type;
	node->Left = left;
